...
using Interpolations, ReactiveBasics
...
function getXYdata(data, runs, is_endpoint, dim1, dim2)
  if (!is_endpoint)
...
function get_ant_lines(data, surf, dim1, dim2, ant_count, scale_factor)
  ...
  for i = 1:ant_count
    ...
    x_spl = Interpolations.interpolate((t_data, ), x_data, Gridded(Linear()))
    y_spl = Interpolations.interpolate((t_data, ), y_data, Gridded(Linear()))
    tmin = minimum(t_data)
    tmax = maximum(t_data)
...
function render_landscape(data, runs, n)
  ...
  XY_signal = map(endpoint_s, dim1_s, dim2_s) do is_endpoint, dim1, dim2
    getXYdata(data, runs, is_endpoint, dim1, dim2)
  end
  ...
  traces_obj = map(ant_count_s) do ant_count
    ant_lines = const_lift(get_ant_lines, data, surface_signal, dim1_s, dim2_s,
    ...
    ant_positions_s = Reactive.flatten(ant_positions_s,
