...
function solveDE(str_initi,str_tspan,str_para,str_eqt)
 ...
 print(tspan)
 bound_l=split(getproperty(boundary,:text,String),"=")
 eval(parse("bound=" * bound_l[2]))
 Out_Put=ode_simulator(getproperty(itera,:text,String),func,length(split(getproperty(inputVari,:text,String),",")), bound, tspan)
 return Out_Put
