...
function get_fill_matrix(Z)
  global plot_type
  is_log = (plot_type == "log") ? true : false
  z_color, color_count = LandscapeColouring.color_landscape(Z, is_log)

  # Get 'Rainbow' colorscheme
  colors = Float64[x for x in linspace(0.0, 1.0, color_count)]
  shuffle(colors)
...
anim = @animate for surf in surfaces
  ...
  # Plot with PyPlot.
  X, Y = ndgrid(X, Y)
  p = Plots.plot(X, Y, Z, st = [:surface], surfcolor=get_fill_matrix(Z),
