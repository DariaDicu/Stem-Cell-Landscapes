...
function get_weight()
    ...
    # Normalise values by dividing by heighest
    max_d = maximum(store_d) # Define here so looping values aren't affected
    max_j = maximum(store_j)
    limit = length(store_d)
    for k in 1:limit
        store_d[k] /= max_d
        store_j[k] /= max_j
    end
    ...
    weight = ones(limit)
    for x in 1:limit
        # Distance: tried to make significant with ^12
        # Jump: make -ve values more significant
        weight[x] *= (1/store_d[x]^12) * (e^-store_j[x])
    end
...
savefig("LeastActionPath_v1.png")
